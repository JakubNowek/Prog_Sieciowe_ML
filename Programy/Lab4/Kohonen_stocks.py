...
def kohonen(p, alpha_0, dane, ile_razy_T=10):
    ...
    # odejmowanie offsetu
    for i in range(len(dane)):
        dane.iloc[i] = (offset-dane.iloc[i])

    # inicjalizacja wektorów reprezentantów
    for j in range(p):
        wr_list.append(1/math.sqrt(N)*np.ones(howManyCols))
...
# wczytywanie csv
stonks = pd.read_csv('KIM_data.csv', usecols=['open', 'high', 'low'])
...
print(stonks)
# # =============================wywołanie======================================= #
# wektory = kohonen(p=3, alpha_0=0.6, dane=stonks)
# # ============================================================================= #
#
...
# for i in range(len(stonks)):
#     stonks.iloc[i] = (offset - stonks.iloc[i])
#
#
# # PLOTTOWANIE dla 3d
# fig = plt.figure()
...
#
# limity = plt.gca()
# limity.set_xlim([-10, 10])
# limity.set_ylim([-10, 10])
# limity.set_zlim([-10, 15])
#
#
